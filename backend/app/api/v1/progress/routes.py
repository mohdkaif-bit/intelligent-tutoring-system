...
@router.post("/update")
async def update_interaction(update: PageInteractionUpdate, user: CurrentUser):
    """Update page interaction for the authenticated user."""
    try:
        memory = LearningMemory(user_id=user.id)

        if update.interaction_type in ("view", "page_view"):
            ...
            memory.update_selection(document_id=update.document_id, page_number=update.page_number)
        elif update.interaction_type == "reframe":
            memory.update_reframe(document_id=update.document_id, page_number=update.page_number)
        elif update.interaction_type == "explanation":
            memory.update_explanation(document_id=update.document_id, page_number=update.page_number)
        ...
        logger.info(
            "Updated %s interaction for page %s (user=%s)",
            update.interaction_type, update.page_number, user.id,
        )
...
@router.get("/account")
async def get_account_progress(user: CurrentUser):
    """Get account-level learning progress for the authenticated user."""
    try:
        memory   = LearningMemory(user_id=user.id)
        progress = memory.get_account_progress()
        logger.info("Retrieved account progress for user=%s", user.id)
        return progress
    except Exception as e:
...
async def get_document_progress(document_id: str, user: CurrentUser):
    """Get progress for a specific document owned by the authenticated user."""
    try:
        memory   = LearningMemory(user_id=user.id)
        progress = memory.get_document_progress(document_id)
        logger.info("Retrieved progress for document %s (user=%s)", document_id, user.id)
...
async def get_revision_suggestions(user: CurrentUser, max_suggestions: int = 10):
    """Get evidence-based revision suggestions for the authenticated user."""
    try:
        memory      = LearningMemory(user_id=user.id)
        suggestions = memory.get_revision_suggestions(max_suggestions=max_suggestions)
        logger.info("Retrieved %d suggestions for user=%s", len(suggestions), user.id)
        return suggestions
    except Exception as e:
...
async def track_page_view(
    ...
    try:
        ...
        logger.info("Tracked page view: doc=%s page=%d user=%s", document_id, page_number, user.id)
        return {"success": True}
    except Exception as e:
...
async def track_question(document_id: str, page_number: int, user: CurrentUser):
    """Shortcut to track questions asked."""
    try:
        memory = LearningMemory(user_id=user.id)
        memory.update_explanation(document_id, page_number)
        logger.info("Tracked question: doc=%s page=%d user=%s", document_id, page_number, user.id)
        return {"success": True}
    except Exception as e:
...
@router.post("/quiz")
async def track_quiz(document_id: str, page_number: int, score: float, user: CurrentUser):
    """Shortcut to track quiz completion. Score: 0.0–1.0."""
    try:
        memory = LearningMemory(user_id=user.id)
        memory.update_quiz(document_id, page_number, score)
        logger.info("Tracked quiz: doc=%s page=%d score=%.2f user=%s", document_id, page_number, score, user.id)
        return {"success": True}
    except Exception as e:
...
async def debug_memory(user: CurrentUser):
...
async def get_document_revisions(document_id: str, user: CurrentUser):
